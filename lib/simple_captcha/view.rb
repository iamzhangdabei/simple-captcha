...
module SimpleCaptcha #:nodoc
  module ViewHelpers #:nodoc
    ...
    def show_update_captcha(options={})
      options[:field_value] = set_simple_captcha_data(options[:code_type])
      @update_captcha_options = 
        {:image => new_captcha_src(options),
         :label => options[:label] || "(type the code from the image)",
         :field => simple_captcha_field(options)}
    ...
    def simple_captcha_image(options={})
      url = 
        simple_captcha_url(
          ...
          :distortion => options[:distortion] || '',
          :time => Time.now.to_i)
      "<img src='#{url}' alt='simple_captcha.jpg' id='captcha_image'/>".html_safe
    end
